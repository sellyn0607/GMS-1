...
import dao.ExamServiceDAOImpl;
import dao.RecordServiceDAOImpl;
import dao.SubjectServiceDAOImpl;
import domain.*;
...
public class ExamServiceImpl implements ExamService{
	...
	public void createExam(ExamBean exam) {
		...
		record.setGrade("");
		record.setAvg("");
		RecordServiceDAOImpl.getInstance().insertRecord(record);
		System.out.println("java : "+arr[0]+"   HTML5 : "+arr[1]+"   R :"+arr[2]+"   SQL :"+ arr[3]+"   Phython : "+arr[4]);
		exam.setScore(exam.getScore().split("/")[0]);
		exam.setSubSeq(SubjectServiceDAOImpl.getInstance().selectSubjectSearch("JAVA").get(0).getSubSeq());
		
		
		exam.setRecoreSeq(RecordServiceDAOImpl.getInstance().selectFirstRowNum());
		ExamServiceDAOImpl.getInstance().insertExam(exam);
